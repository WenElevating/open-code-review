...
`llm.protocol` 用于选择接口类型：

- `anthropic`：Anthropic Messages API，URL 为基础地址时自动追加 `/v1/messages`
- `anthropic_exact`：Anthropic Messages API，严格使用 `llm.url`，不自动追加路径
- `openai`：OpenAI Chat Completions API，URL 为基础地址时自动追加 `/chat/completions`
- `openai_exact`：OpenAI Chat Completions API，严格使用 `llm.url`，不自动追加路径

`llm.use_anthropic` 是兼容旧配置的布尔开关；同时配置时，`llm.protocol` 优先生效。
